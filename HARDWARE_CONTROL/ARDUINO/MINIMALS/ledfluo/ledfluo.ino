...
// define RGB struct for setting the intensity of the LED
struct RGB {
  byte r;
  byte g;
  byte b;
...
const char *COMMANDSET[nCommands] = {PXL", "CLEAR"};
const char *INSTRUCTS[nCommands] = {"1", "4", "4", "4", "8", "6", "3", "3", "3", "3", "0"};
...
void executeCommand(int nINST)
{
  ...
  else if (strcmp(CMD, COMMANDSET[0]) == 0)
  {
   strlcat(outBuffer, "Pressed PXL", outBufLen);
    ...
    digitalWrite(LED, rgb.r);
    analogWrite(ledPin, val / 4);
    strlcat(outBuffer, "FLUO LED on", outBufLen);
  }
  else if (strcmp(CMD, COMMANDSET[1]) == 0)
  {
  digitalWrite(LED, LOW);
  strlcat(outBuffer, "FLUO LED off", outBufLen);
  }
  else
  {
...
void setup()
{
  ...
  // Flash LED for a second to see if it works
