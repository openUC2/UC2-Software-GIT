...
std::string SETUP = "S001";      //S006->Aurelie; S004->Barbora
...
// ~~~~  Wifi  ~~~~
const char *ssid = "WLAN-PANH68"; //"UC2_wifi004";
const char *password = "3032889043762777"; //"_lachmannUC2";
...
#define IS_MQTT_SERVER_EQUALS_ROUTER 0
  ...
  String gatewayIP;
#else 
  const char *MQTT_SERVER = "192.168.2.112"; //"21.3.2.152";
#endif
...
void callback(char *topic, byte *message, unsigned int length)
{
    ...
    if (std::string(topic) == stopicREC)
    {
        ...
        {
            //setNA(INSTS[0]);
            if((int)INSTS[0]==1) M5.Lcd.fillScreen(WHITE);
            if((int)INSTS[0]==2) M5.Lcd.fillScreen(RED);
            if((int)INSTS[0]==3) M5.Lcd.fillScreen(BLUE);
            if((int)INSTS[0]==4) M5.Lcd.fillScreen(BLACK);
            if((int)INSTS[0]==5) M5.Lcd.fillScreen(GREEN);
