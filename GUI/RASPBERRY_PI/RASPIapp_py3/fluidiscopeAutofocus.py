...
def autofocus_routine(self, camStats=None):
    ...
    if not fg.config['autofocus']['use_coarse_only']:
        ...
        sharpness_fine, pos_fine = autofocus_scan(self, names=image_name_template, rawCapture=rawCapture, smethod=smethod, pos_start=pos_optimum_coarse, pos_min=pos_min, pos_max=pos_max, max_steps=max_steps, save_im=save_im)
...
def autofocus_getMeasure(im, sharpness, tproc):
    ...
    tprocstart = time()
    if 1: #fg.config['autofocus']['scan_method'] == 'std':
    ...
    logger.debug("Sharpness of Autofocus is  {}".format(sharpness[-1]))
